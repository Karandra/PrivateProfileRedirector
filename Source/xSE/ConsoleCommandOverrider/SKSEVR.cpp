...
namespace PPR
{
	ObScriptCommand* ConsoleCommandOverrider_SKSEVR::FindCommand(KxDynamicStringRefA fullName) const
	{
	...
	bool ConsoleCommandOverrider_SKSEVR::OnCommand(void* paramInfo, void* scriptData, TESObjectREFR* thisObj, void* containingObj, void* scriptObj, void* locals, double* result, void* opcodeOffset)
	{
		if (scriptData)
		{
	...
	bool ConsoleCommandOverrider_SKSEVR::OverrideCommand(KxDynamicStringRefA commandName, KxDynamicStringRefA commandHelp)
	{
		if (ObScriptCommand* command = FindCommand(commandName))
		{
			...
			{
				auto& instance = *SEInterface::GetInstance().GetConsoleCommandOverrider<ConsoleCommandOverrider_SKSEVR>();
